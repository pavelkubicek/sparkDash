...
### Changed
- **Machine-agnostic Model Launcher** — model scripts no longer run on the dashboard's own host. Each card names the Spark where its repo lives (`sparkId`, chosen in the card's edit dialog, required by start/stop/restart/logs); every script, `docker ps`, repo-URL detection and `/v1/models` liveness probe now executes or addresses THAT Spark over the existing SSH connection (password auth reuses the encrypted stored secrets; key auth works unchanged). An exclusive stop→start that crosses machines splits into one chained command per Spark, run in order; the job timeout caps the whole job, and cancelling kills the SSH session while containers already handed to dockerd keep running. `nsenter` execution survives only as the fallback for a genuinely local Spark without SSH config. Moving sparkDash to another box is now a deployment step, not a code change — set `MODEL_REPOS_BASE` to the repos base as it exists **on the target Sparks** and every card keeps working.

### Fixed
- **AI Proxy / Dev Engine bridges after dashboard migration** — both bridges fetched their services at `127.0.0.1`, and the observer / engine-webui "jump" links reused the dashboard's own hostname, so every integration broke the moment sparkDash moved off the machine hosting them. New `AI_PROXY_HOST`, `DEV_ENGINE_API_HOST` and `DEV_ENGINE_WEBUI_HOST` env vars name the service machine (unset = co-located loopback/request-host defaults, single-host installs unchanged); browser-facing links now carry the configured host instead of the dashboard's.
...
- **Scheduler ran 2 h off the operator's clock** — `config/scheduler.json` carried `"tz": "UTC"`, so schedule windows entered in Europe/Prague wall-clock (e.g. a DeepSeek 18:00 start) fired at 20:00 local. The UTC→operator-zone coercion applied only in `loadSchedulerConfig`; `updateSchedulerConfig` (the API path) and the un-loaded `getSchedulerConfig` accepted and persisted `UTC`. All three paths now resolve `UTC` / a missing zone to `MODEL_SCHEDULER_TZ` (Europe/Prague), so a `UTC` write can never stick as the effective schedule zone.
...
- **CPU temperature sensor selection** — the collector now reports temperature for all kinds. On DGX Sparks (GB10/ARM) the value is an `acpitz` board/SoC zone (not a CPU core temp), and the CPU panel labels it **"SoC temp"** so it is not mistaken for a CPU core reading. On dedicated GPU hosts (`kind: host`), the hottest real `coretemp`/`k10temp`/`zenpower` input is used (previously the first `temp*_input`, which on multi-sensor x86 picked an arbitrary core); `acpitz`/thermal zones remain a last-resort board fallback. Remote Sparks now read the same sensors over SSH.
